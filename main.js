...
function startMine(config) {
  ...
  writeStream.write(
    JSON.stringify({ id, memory, thread, tmp, output, totalPlots })
  );
  ...
  child.on("close", function (code) {
    ...
    if (killed) {
      ...
      fs.renameSync(
        fileLogName,
        fileLogName.replace("output/", "output/[DONE]")
      );
...
["SIGINT", "SIGTERM", "SIGQUIT"].forEach((signal) =>
  process.on(signal, () => {
    ...
    new Array(...processLogMap.keys()).forEach((pid) => {
      ...
      fs.renameSync(
        fileLogName,
        fileLogName.replace("output/", "output/[KILLED]")
      );
...
configs.forEach((config, index) => {
  const { totalPlots, concurrent } = config;
  counter.set(index, Number(totalPlots));
  for (let i = 0; i < concurrent; i++) {
    startMine({ id: index, ...config });
  }
});
